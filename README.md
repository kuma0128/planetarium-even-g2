...
Image updates pause while the OS exit dialog or contextual menu covers the app. `FOREGROUND_ENTER_EVENT` (4) opens this overlay; `FOREGROUND_EXIT_EVENT` (5) dismisses it. Selecting **No** resumes the complete current view and allows the next double-tap to open the exit dialog again, without reconnecting. Head tracking requires calibration again. Transfers already waiting for the host cannot be cancelled, but their late results do not close the resumed session. Confirmed system exit and disconnect still stop the session. See the [official overlay lifecycle documentation](https://hub.evenrealities.com/docs/build/contextual-menu).
