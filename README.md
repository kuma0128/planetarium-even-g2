...
**Download sensor log** saves the last 600 raw samples with monotonic arrival times from session start, calibration markers, selected format, and the latest host-transfer duration as JSON. Coordinates are not included or uploaded. `P100` is the selected SDK pacing code; it is not a claim of 100 Hz. The UI reports the observed arrival rate. Tracking requests display updates at most every 100 ms, keeps only the latest waiting frame, and skips unchanged tiles/captions. Real optical refresh rate and latency still need measurement on G2; host acceptance does not establish either.
